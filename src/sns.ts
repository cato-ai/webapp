...
export const push_to_sns = async (user) => {
  ...
  const verification_link = "http://demo.sampurna.xyz/v1/user/verify/self"; //create api endpoint here with required details, bcrypt for the hash of the username and password, a long with timestamp to cehck if it's expired or not
  try {
